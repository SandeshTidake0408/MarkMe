...
function student_location() {
    ...
    const successCallback = (position) => {
        ...
        S_Altitude = position.coords.altitude || null;
...
// Timer functionality - uncomment if needed
// async function get_time() {
//     try {
//         const result = await api.get(`/feed/timer/${id}`);
...
markme_btn.addEventListener("click", async () => {
    ...
    try {
        const result = await api.post("/mark", {
            key: key,
            subject: subject,
            rollNo: rollno,
            email: user_mail,
        ...
        messageEl.textContent = result.data.msg;
    } catch (error) {
        const messageEl = document.getElementById("message");
        messageEl.className = "message error";
        if (error.response) {
            messageEl.textContent = error.response.data.msg;
        } else {
